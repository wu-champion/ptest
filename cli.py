...
{get_colored_text('Examples:', 93)}
  ...
  ptest obj status
    ...
    # init command - 高优先级
    init_parser = subparsers.add_parser('init', help=get_colored_text('Initialize test environment', 92))
    init_parser.add_argument('--path', required=True, help='Path for test environment')
    
    # config command - 高优先级
    config_parser = subparsers.add_parser('config', help=get_colored_text('Configuration management', 92))
    config_parser.add_argument('--file', help='Configuration file path')
    
    # obj commands - 高优先级
    obj_parser = subparsers.add_parser('obj', help=get_colored_text('Manage test objects', 92))
    ...
    # tool commands - 中优先级
    tool_parser = subparsers.add_parser('tool', help=get_colored_text('Manage tools', 93))
    ...
    # case commands - 中优先级
    case_parser = subparsers.add_parser('case', help=get_colored_text('Manage test cases', 93))
    ...
    # report command - 低优先级
    report_parser = subparsers.add_parser('report', help=get_colored_text('Generate test reports', 96))
    report_parser.add_argument('--format', choices=['html', 'json'], default='html', 
                              help='Report format')
    
    # status command - 高优先级
    status_parser = subparsers.add_parser('status', help=get_colored_text('Show overall status', 92))
...
def main():
    ...
    try:
        if args.command == 'init':
            ...
            config_file = env_manager.test_path / 'ptest_config.json' if env_manager.test_path else args.file
            if config_file:
            ...
            if args.tool_action == 'install':
                ...
                print(result)
            elif args.tool_action == 'restart':
                result = tool_manager.restart(args.name)
                print(result)
