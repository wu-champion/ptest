...
class TestEnvironmentMigration(unittest.TestCase):
    ...
    def test_progress_tracking(self):
        ...
        self.assertEqual(len([c for c in progress_calls if c[0] == "update"]), 5)  # 5步更新
        self.assertEqual(len([c for c in progress_calls if c[0] == "complete"]), 1)  # 最终完成
    ...
    def test_migration_error_handling(self):
        ...
        # 测试目标环境已存在
        self.source_engine.create_environment(
            "test_env_existing",
            {"python": "3.12"}
        )
